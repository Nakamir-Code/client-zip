// source: https://github.com/ARGA-Genomes/arga-frontend/blob/main/public/download-worker.js
importScripts(
            ...
            async function* namedInput() {
              for await (const response of dlstream) {
                if (!response || !response.url) {
                  console.warn("Skipping null or invalid response");
                  continue;
                }
            
                if (!response.ok) {
                  console.warn(`Skipping file due to HTTP error: ${response.status} for ${response.url}`);
                  continue;
                }
            
                const fileName = new URL(response.url).pathname.split("/").pop();
                if (!fileName) {
                  console.warn("Skipping file with no valid name");
                  continue;
                }
